    ...
    <div>
      ...
      {inquiries.map(inq => <Card key={inq.id}>
        ...
        <div style={{ marginTop: 10 }}>
          <p style={{ fontSize: 12, fontWeight: 700, color: COLORS.primary, marginBottom: 6 }}>回答文：</p>
          <textarea
            placeholder="回答を入力してください"
            defaultValue={inq.aiReply || ""}
            onBlur={async (e) => {
              await updateDoc(doc(db, "inquiries", inq.id), { aiReply: e.target.value, status: "回答済" });
            }}
            rows={3}
            style={{ ...inputStyle, resize: "vertical" }}
          />
          <p style={{ fontSize: 11, color: COLORS.textMuted, marginTop: 4 }}>入力後、フォーカスを外すと自動保存されます</p>
        </div>
      </Card>)}
    </div>
  );
