...
const INITIAL_NOTICES = [
  { id: 1, title: "令和7年度 第1回役員会開催のお知らせ", date: "2026-05-20", urgent: true, body: "日時：令和7年5月20日（火）19:00〜\n場所：地区センター 第2会議室\n議題：年間活動計画・予算審議" },
  { id: 2, title: "夏祭り準備委員会 メンバー募集", date: "2026-05-15", urgent: false, body: "8月の夏祭りに向けて準備委員会を設置します。参加希望の方はお問い合わせ欄よりご連絡ください。" },
];
const INITIAL_MEETINGS = [
  { id: 1, title: "令和7年度 第1回役員会", date: "2026-05-20", time: "19:00", place: "地区センター 第2会議室", agenda: "年間活動計画・予算審議", responses: { "田中会長": "参加", "山田副会長": "参加", "鈴木書記": "未回答", "佐藤会計": "欠席" } },
];
const INITIAL_REPORTS = [
  { id: 1, title: "高齢者訪問活動（4月）", date: "2026-05-01", author: "鈴木書記", content: "今月は15世帯を訪問しました。皆さん元気に過ごされています。", image: null },
];
const INITIAL_CHATS = [
  { id: 1, user: "田中会長", text: "皆さん、5月の役員会よろしくお願いします！", time: "10:30" },
  { id: 2, user: "山田副会長", text: "承知しました。資料は前日までに送ります。", time: "10:45" },
];
const MEMBERS = ["田中会長", "山田副会長", "鈴木書記", "佐藤会計"];
...
function NoticesPage() {
  ...
  const post = () => { if (!form.title.trim()) return; setNotices([{ id: Date.now(), ...form, date: new Date().toISOString().slice(0,10) }, ...notices]); setForm({ title:"", body:"", urgent:false }); setShowForm(false); };
  if (selected) return (
    <div>
...
function MeetingsPage() {
  ...
  const respond = (id, ans) => { setMeetings(ms => ms.map(m => m.id===id ? {...m, responses:{...m.responses,[myName]:ans}} : m)); };
  const add = () => { if(!form.title||!form.date) return; setMeetings([{id:Date.now(),...form,responses:{}}, ...meetings]); setForm({title:"",date:"",time:"",place:"",agenda:""}); setShowForm(false); };
  if (selected) {
...
function ReportsPage() {
  ...
  const post = () => { if(!form.title.trim()) return; setReports([{id:Date.now(),...form,date:new Date().toISOString().slice(0,10),image:img},...reports]); setForm({title:"",author:"",content:""}); setImg(null); setShowForm(false); };
  return (
    <div>
      <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14 }}>
...
function ChatPage() {
  ...
  const send = () => { if(!text.trim()) return; const now=new Date(); setChats([...chats,{id:Date.now(),user:myName,text:text.trim(),time:`${now.getHours()}:${String(now.getMinutes()).padStart(2,"0")}`}]); setText(""); };
  return (
    <div style={{ display:"flex",flexDirection:"column" }}>
      ...
      <div style={{ overflowY:"auto",maxHeight:340,marginBottom:12,paddingRight:4 }}>
        {chats.map(c=>{ const isMe=c.user===myName; return (
...
function InquiryPage() {
  const [inquiries, setInquiries] = useState([
    { id:1, name:"地域住民A", subject:"福祉サービスについて", message:"高齢の親の見守りサービスについて教えてください。", date:"2026-05-11", status:"回答済", aiReply:"" },
    { id:2, name:"地域住民B", subject:"夏祭りボランティア参加", message:"夏祭りにボランティアとして参加したいのですが。", date:"2026-05-12", status:"未回答", aiReply:"" },
  ]);
  const [form, setForm] = useState({ name:"", subject:"", message:"" });
  const [sent, setSent] = useState(false);
  const [loading, setLoading] = useState(null);
  const submit = () => { if(!form.name||!form.message) return; setInquiries([{id:Date.now(),...form,date:new Date().toISOString().slice(0,10),status:"未回答",aiReply:""},...inquiries]); setForm({name:"",subject:"",message:""}); setSent(true); setTimeout(()=>setSent(false),3000); };
  const genAI = async (inq) => {
    setLoading(inq.id);
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages", { method:"POST", headers:{"Content-Type":"application/json"}, body: JSON.stringify({ model:"claude-sonnet-4-20250514", max_tokens:1000, messages:[{ role:"user", content:`あなたは町の福祉会の担当者です。以下のお問い合わせに丁寧な日本語で200文字以内の回答文を作成してください。\n件名：${inq.subject}\n内容：${inq.message}` }] }) });
      const data = await res.json();
      const reply = data.content?.[0]?.text || "生成できませんでした。";
      setInquiries(qs => qs.map(q => q.id===inq.id ? {...q, aiReply:reply} : q));
    } catch { setInquiries(qs => qs.map(q => q.id===inq.id ? {...q, aiReply:"エラーが発生しました。"} : q)); }
    setLoading(null);
  };
  return (
    <div>
...
function EmergencyPage() {
  ...
  const [sent, setSent] = useState(false);
  const [history, setHistory] = useState([{ id:1, type:"緊急招集", detail:"明日の防災訓練の集合時間を8時に変更します。", date:"2026-05-10 14:30", sender:"田中会長" }]);
  const TYPES = ["安否確認","緊急招集","災害情報","その他"];
  const send = () => { if(!form.detail.trim()) return; setHistory([{id:Date.now(),...form,date:new Date().toLocaleString("ja-JP"),sender:"現在のユーザー"},...history]); setSent(true); setForm({type:"安否確認",detail:""}); setTimeout(()=>setSent(false),4000); };
  return (
    <div>
